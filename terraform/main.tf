...
# Project: Pastor Wants A Wife (DStv · Mzansi Magic · Channel 161)
# ------------------------------------------------------------------------------
...
resource "aws_kms_key" "media_key" {
  description             = "KMS Key for Pastor Wants A Wife media asset encryption"
  deletion_window_in_days = 30
  enable_key_rotation     = true

  tags = {
    Name = "pastor_wants_a_wife_broadcast_workflow-kms-key"
  }
}

# 2. S3 Media Vault Bucket
resource "aws_s3_bucket" "media_vault" {
  bucket        = "nwhite-pastor-wants-a-wife-broadcast-workflow-${var.environment}"
  force_destroy = false
}
...
resource "aws_iam_role" "post_production_role" {
  name = "pastor_wants_a_wife_broadcast_workflow-post-production-role"
...
resource "aws_iam_policy" "post_production_policy" {
  name        = "pastor_wants_a_wife_broadcast_workflow-post-production-policy"
  description = "Scoped S3 and KMS permissions for Pastor Wants A Wife post-production workflow"
...
resource "aws_cloudwatch_log_group" "workflow_logs" {
  name              = "/aws/media-pipeline/pastor_wants_a_wife_broadcast_workflow"
